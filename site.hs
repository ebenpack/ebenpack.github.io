                ...
                archiveCtx =
                    listField
                            ...
                            (  field
                                    "date"
                                    ( return
                                    . formatTime defaultTimeLocale "%B %0Y"
                                    . fst
                                    . itemBody
                                    )
                            <> listFieldWith
            ...
            let paginateCtx = paginateContext page pageNum
                indexCtx =
                    listField "posts" teaserCtx (return posts)
                        ...
                        <> defaultContext
            makeItem ""
                >>= loadAndApplyTemplate "templates/index.tmpl"   indexCtx
...
slugRoute = metadataRoute makeRoute
  where
    ...
    makeRoute md = case lookupString "slug" md of
...
groupChronologicalItems items = do
    withTime <- forM items $ \item -> do
        ...
        return (utc, [item])
    return $ fmap merge $ groupBy compareTime $ sortBy
        (flip (comparing fst))
        withTime
  where
...
paginateContextPlus page currentPage =
    paginateContext page currentPage <> mconcat
...
makeInfoItem (n, i) = getRoute i >>= makeInfo
  where
    ...
    makeInfo filepath = case filepath of
